MODULE zdftke
   ...
   REAL(wp) ::   rn_ebbice = 3.75_wp        ! coefficient of the surface input of tke under ice
   INTEGER  ::   nn_havti = 0               ! background modified under ice or not (0/1)
...
CONTAINS
   ...
   INTEGER FUNCTION zdf_tke_alloc()
      ...
      ALLOCATE(                                                                    &
         ...
         &   ziceavtb(jpi,jpj     ),    STAT= zdf_tke_alloc                        )
         !
...
!{ DRAKKAR : no LC under ice 
!                 en(ji,jj,jk) = en(ji,jj,jk) + rdt * ( zwlc * zwlc * zwlc ) / zhlc(ji,jj) * tmask(ji,jj,jk)
                  en(ji,jj,jk) = en(ji,jj,jk) + (1.- fr_i(ji,jj)) * rdt * ( zwlc * zwlc * zwlc ) / zhlc(ji,jj) * tmask(ji,jj,jk)
!}
               END DO
            END DO
         END DO
...
!CDIR NOVERRCHK
            DO ji = fs_2, fs_jpim1   ! vector opt.
               ...
               !{ DRAKKAR 
               avt  (ji,jj,jk) = MAX( zav, ziceavtb(ji,jj) * avtb(jk) ) * tmask(ji,jj,jk)
               !}
...
!!gm              zpdlr = MAX(  0.1_wp,  ri_crit / MAX( ri_crit , zri )  )
                  !{ DRAKKAR 
                  avt(ji,jj,jk)   = MAX( zpdlr * avt(ji,jj,jk), ziceavtb(ji,jj) * avtb(jk) ) * tmask(ji,jj,jk)
                  !}
# if defined key_c1d
...
# endif
   ...
   SUBROUTINE zdf_tke_init
      ...
      INTEGER ::   ji, jj, jk   ! dummy loop indices
      !!
      !{ DRAKKAR : add rn_ebbice and nn_havti in the namelist 
      NAMELIST/namzdf_tke/ rn_ediff, rn_ediss , rn_ebb   , rn_ebbice, rn_emin  ,   &
         &                 rn_emin0, rn_bshear, nn_mxl ,  ln_mxl0  ,   &
         &                 rn_mxl0 , nn_pdl   , nn_havti, ln_lc  , rn_lc    ,   &
