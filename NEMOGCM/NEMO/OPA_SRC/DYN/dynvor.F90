...
#  include "vectopt_loop_substitute.h90"
   ...
   !! $Id: dynvor.F90 3231 2011-12-21 09:11:11Z smasson $
...
!{ JMM for EEN test as ND
      REAL(wp) ::   zsmsk                              ! local scalars
!}
      !                                                           !  3D workspace 
      REAL(wp), POINTER    , DIMENSION(:,:  )         :: zwx, zwy, zwz
      ...
      IF( kt == nit000 .OR. lk_vvl ) THEN      ! reciprocal of e3 at F-point (masked averaging of e3t)
         DO jk = 1, jpk
            DO jj = 1, jpjm1
               DO ji = 1, jpim1
!{ JMM
                  zsmsk= (tmask(ji,jj+1,jk) + tmask(ji+1,jj+1,jk) + tmask(ji,jj  ,jk) + tmask(ji+1,jj  ,jk) )
...
!                 IF( ze3f(ji,jj,jk) /= 0._wp )   ze3f(ji,jj,jk) = 1._wp / ze3f(ji,jj,jk)
                  ...
                  IF( ze3f(ji,jj,jk) /= 0._wp )   ze3f(ji,jj,jk) = zsmsk / ze3f(ji,jj,jk)
!}
               END DO
            END DO
         END DO
