MODULE limwri_daily_2
   ...
   PUBLIC   lim_wri_daily_2         ! called by sbc_ice_lim_2
#endif
...
#   include "vectopt_loop_substitute.h90"
   ...
   !! $Id: limwri_daily_2.F90 3200 2011-12-07 15:00:49Z cbricaud $
   !! Software governed by the CeCILL licence (NEMOGCM/NEMO_CeCILL.txt)
   !!----------------------------------------------------------------------

CONTAINS
   INTEGER FUNCTION lim_wri_daily_alloc_2()
      !!-------------------------------------------------------------------
      !!                  ***   ROUTINE lim_wri_daily_alloc_2  ***
      !!-------------------------------------------------------------------
      ALLOCATE( ndex51(jpij), zcmo(jpi,jpj,jpnoumax), STAT=lim_wri_daily_alloc_2)
      !
      IF( lk_mpp               )   CALL mpp_sum ( lim_wri_daily_alloc_2 )
      IF( lim_wri_daily_alloc_2 /= 0 )   CALL ctl_warn('lim_wri_daily_alloc_2: failed to allocate array ndex51')
      !
   END FUNCTION lim_wri_daily_alloc_2
...
# if defined key_dimgout
   ...
   !!----------------------------------------------------------------------
# include "limwri_dimg_daily_2.h90"
# else
   SUBROUTINE lim_wri_daily_2( kt )
      !!-------------------------------------------------------------------
      !!                    ***   ROUTINE lim_wri_daily_2  ***
      !!                
      ...
      !
   END SUBROUTINE lim_wri_daily_2
     
#endif     

   SUBROUTINE lim_wri_daily_init_2
      !!-------------------------------------------------------------------
      ...
      !!-------------------------------------------------------------------
      !
      IF( lim_wri_daily_alloc_2() /= 0 ) THEN      ! allocate lim_wri arrrays
         CALL ctl_stop( 'STOP', 'lim_wri_daily_init_2 : unable to allocate standard arrays' )   ;   RETURN
      ENDIF
      ...
      IF(lwp) THEN
         WRITE(numout,*)
         WRITE(numout,*) 'lim_wri_daily_init_2 : Ice parameters for outputs'
         WRITE(numout,*) '~~~~~~~~~~~~~~'
      ...
      ENDIF
      !    
   END SUBROUTINE lim_wri_daily_init_2
   ...
   !!----------------------------------------------------------------------
CONTAINS
   SUBROUTINE lim_wri_daily_2          ! Empty routine
   END SUBROUTINE lim_wri_daily_2
#endif

   !!======================================================================
END MODULE limwri_daily_2
